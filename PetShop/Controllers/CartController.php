...
class CartController
{
    ...
    function add_cart()
    {
        ...
        $count = 0;
        if (isset($_SESSION['sanphams'][$id])) {
            $arr = $_SESSION['sanphams'][$id];
            $arr['SoLuong'] = $arr['soluong'] + 1;
            $arr['ThanhTien'] = $arr['soluong'] * $arr["DonGia"];
            $_SESSION['sanphams'][$id] = $arr;
        } else {
            $arr['MaSP'] = $data['MaSP'];
    ...
    {
        $arr = $_SESSION['sanphams'][$_GET['id']];
        $arr['SoLuong'] = $arr['SoLuong'] + 1;
        $arr['ThanhTien'] = $arr['SoLuong'] * $arr["DonGia"];
        $_SESSION['sanphams'][$_GET['id']] = $arr;
        header('Location: ?act=cart#dxd');
    }
    function delete_cart()
    {
        $arr = $_SESSION['sanphams'][$_GET['id']];
        if ($arr['SoLuong'] == 1) {
            unset($_SESSION['sanphams'][$_GET['id']]);
        } else {
            $arr = $_SESSION['sanphams'][$_GET['id']];
            $arr['SoLuong'] = $arr['SoLuong'] - 1;
            $arr['ThanhTien'] = $arr['SoLuong'] * $arr["DonGia"];

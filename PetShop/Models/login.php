...
class Login extends Model
{
    ...
    function login_action($data)
    {
        ...
        if ($login !== NULL) {
            if ($login['MaQuyen'] == 2) {
                $_SESSION['isLogin_Admins'] = true;
                $_SESSION['logins'] = $login;
            } else {
                if ($login['MaQuyen'] == 3) {
                    $_SESSION['isLogin_Nhanviens'] = true;
                    $_SESSION['logins'] = $login;
                } else {
                    $_SESSION['isLogins'] = true;
                    $_SESSION['logins'] = $login;
                }
            }
    ...
    function logout()
    {
        if (isset($_SESSION['isLogin_Admins'])) {
            unset($_SESSION['isLogin_Admins']);
            unset($_SESSION['logins']);
        }
        if (isset($_SESSION['isLogin_Nhanviens'])) {
            unset($_SESSION['isLogin_Nhanviens']);
            unset($_SESSION['logins']);
        }
        if (isset($_SESSION['isLogins'])) {
            unset($_SESSION['isLogins']);
            unset($_SESSION['logins']);
        }
    ...
    function update_account($data)
    {
        ...
        $query = "UPDATE NguoiDung SET  $v   WHERE  MaND = " . $_SESSION['logins']['MaND'];
